...
// ICN CHNAGED
import { icnService } from '../services/icnService';
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  // ICN CHNAGED
  const [icnOpenPostsCount, setIcnOpenPostsCount] = useState(0);
  ...
  const navigation = [
    ...
    // ICN CHNAGED
    { name: 'ICN Exchange', href: '/dashboard/icn',         icon: Network },
    ...
    { name: 'Inventory Update', href: `/dashboard/inventory-update${user?.pharmaID ? `/${user.pharmaID}` : ''}`, icon: UploadIcon },
  ...
  const loadIcnBadgeCount = async () => {
    // ICN CHNAGED
    if (!user?.id) return;
    try {
      const posts = await icnService.getExchanges({ status: 'OPEN', mine: false });
      setIcnOpenPostsCount(posts.length);
    } catch (err) {
      console.error('ICN badge fetch failed', err);
    }
  };
  ...
  useEffect(() => {
    loadIcnBadgeCount();
    // ICN CHNAGED
    const interval = setInterval(loadIcnBadgeCount, 10000);
    return () => clearInterval(interval);
  }, [user?.id]);
        ...
        <nav className="flex-1 px-4 space-y-2 pb-6">
          ...
          const isActive = item.exact
           ? location.pathname === item.href
            ...
            // ICN CHNAGED
            const badge = item.name === 'Orders' ? notifications.length : (item.name === 'ICN Exchange' ? icnOpenPostsCount : null);
            return (
